...
> **当前状态**：已完成并关闭（M1.0 至 M1.4 全部通过；下一阶段为 M2）
>
...
M1.4 自动化证据见下一节，最终用户真实音频验收结果见第 10.1 节。

### 8.3 M1.4 自动化与正式产品证据

M1.4 已把音频宿主和 Runtime 的实时计数器贯通到产品状态与界面，控制线程可读取捕获/
渲染帧、欠载、溢出、积压丢帧、无效或重叠回调、非有限输入、有限值饱和和无效处理调用。
宿主新增 bool 与 `UInt64` 原子的 lock-free 能力检查，Runtime 继续检查其发布和诊断原子。
`scripts/verify-m1-realtime.sh` 对 21 个明确枚举的回调与实时 helper 做源码审计，并检查同两个
实现文件中按裸函数名直接调用的本地 helper 是否也在审计集合中；它拒绝常见分配入口、锁、
等待、日志、文件/网络 I/O、dispatch、Objective-C 消息和异常。该门禁不声称解析 C++ 重载、
头文件内联调用或提供编译器/二进制级证明，最终证据同时包含人工源码复核和 lock-free 能力检查。

新增真实 callback 线程与控制发布线程并发的 10,000 次候选压力用例：发布前完成 callback
启动握手，每 100 次发布要求 callback 取得新进展，并用两个声道的固定比例不变量检查块内
目标一致性；同时保留既有确定性交错测试。新增 IO/Runtime 诊断传播、宿主原子能力和 Repair
最终目录同步不确定/同代次 Retry
故障覆盖。完整 hostless RuntimeTests 串行执行 156 项通过、零失败、零跳过；五 target 无签名
构建、隔离审计、实时审计、工程元数据、脚本语法和静态差异检查通过。

默认 `EqualizerAU` scheme 已切换到独立 M1 target。M1 正常产物写入 configuration 目录的
`M1/` 子目录；同一 `SYMROOT` 中先后构建 M0 与 M1 后，两份 app 路径不同且 M0 可执行文件
哈希不变。Release archive 产物为 universal
`EqualizerAU.app`，可执行文件 `EqualizerAU`，bundle ID `com.ruimingchen.EqualizerAU`；Apple
Development 签名、designated requirement、hardened runtime、archive plist 和严格签名验证
均通过。内部 target 和 Swift 模块名保留 `EqualizerAUM1` 以维持 M0/M1 工程隔离。

以上自动化步骤未启动 GUI、真实音频、HAL 路线或 hosted 测试。用户随后按第 10 节脚本
完成真实音频验收并明确报告通过；该人工证据与自动化证据共同关闭 M1。
...
| 实时审计 | 回调无动态分配或动态对象创建、锁、等待、日志及配置 I/O；允许只包装原子操作的有界栈上 RAII guard；活动链、回调状态和音效总开关原子均为 lock-free |
...
2026-07-20，用户收到包含音量安全准备、`-12 dB` 与 `-6 dB` 听感、运行中平滑 Save、
节点停用、音效总开关、退出重启后的 UUID/顺序/参数恢复、至少三次 Start/Stop，以及
30 秒编辑窗口实时计数零增量判定的完整人工验收脚本后，明确回复“通过”。据此记录第 10 节
所列场景整体通过，M1 的真实音频验收条件已满足。

该反馈是用户的整体通过结论；未提供输出设备型号、逐步骤文字记录或原始诊断计数值，本文
不补写这些未报告信息。
...
以上十项退出条件均已满足，M1 于 2026-07-20 关闭，可以进入 M2。
