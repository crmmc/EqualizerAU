...
用户通过固定中心频率的频段控件调整增益。参数变化应及时生效，不要求重启
应用或重新开始播放。
...
- 列表底部 Add 菜单追加节点；Paste 可在首个选中节点前插入；新增节点不改变当前选择；
...
- 采样率变化后仍保持正确频响；
- 参数变化不得产生明显爆音或长期中断；
- 首版频段与增益范围遵循 8.3 的固定 Graphic EQ 契约。
...
- 可滚动处理链；
- 添加节点入口；
- 通用可展开节点行，以及节点启用或停用、删除、拖拽排序和参数编辑；
- 当前输出、采样率和处理延迟等基础信息。

### 8.3 Graphic EQ

Graphic EQ 首版使用固定 15 个中心频率：`25, 40, 63, 100, 160, 250, 400, 630, 1000,
1600, 2500, 4000, 6300, 10000, 16000 Hz`。每段允许在 `-24...+24 dB` 范围内以 `0.1 dB`
步进调整，并同时支持直接数值输入。当前采样率无法处理的高频段必须明确标识，不得静默映射
到其他频率。
