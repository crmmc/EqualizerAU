...
- 用**前置放大**调整整体增益。
- 用**任意点图形均衡器**编辑响应曲线，并编译为 16,384 taps 最小相位 FIR。
...
- 卷积支持采样率 8–768 kHz 的 RIFF/WAVE PCM 8/16/24/32-bit 或 Float32，不限制文件时长或 taps；源采样率必须与当前输出匹配，超过 8 秒时显示性能下降警告，末尾精确零值不进入 Runtime kernel。
