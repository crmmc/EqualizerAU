...
- 前置放大、任意点图形均衡器（最小相位 FIR，长度随输出采样率缩放：≤48 kHz 为 16,384 taps，96/192 kHz 保持同等频率分辨率）、外部 WAV 卷积脉冲响应（源路径引用，不复制文件）。
...
- Graphic EQ FIR 长度随输出采样率缩放（ADR-0020）：`N(Fs) = 16384 × 2^max(0, ceil(log2(Fs/48000)))`。
- 卷积 IR 性能下降橙色警告门限为 30 秒（依据 M1 Release dense probe；常见长 IR 不误报）。
