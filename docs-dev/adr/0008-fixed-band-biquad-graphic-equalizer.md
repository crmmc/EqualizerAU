...
| 属性 | 内容 |
|---|---|
| 状态 | 已接受并实现；决策 6 已被 2026-07-31 用户纠正否决，替代设计见 Accepted ADR-0018 |
| 日期 | 2026-07-21 |
...
6. **历史决策，已不再适用**：Processing 旁路使用独立 10 ms dry/wet 混合；旁路期间仍推进
   wet chain 和 biquad 状态，恢复时从热状态淡入。2026-07-31 真实长 IR 验收证明该方案无法从
   wet overload 中逃生；Accepted [`ADR-0018`](0018-computational-processing-bypass.md) 已实现替代路径。
...
| 旁路时冻结滤波状态 | 历史拒绝理由已由 ADR-0018 重新评估：真实 computational bypass 停止 wet，重新启用必须安装 fresh Prepared，而不是恢复冻结状态 |
