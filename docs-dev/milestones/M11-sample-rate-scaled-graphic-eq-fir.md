...
结论：在 M1 级机器上，1/2/4/8 声道的 48/96 kHz 与 1/2/4 声道的 192 kHz dense EQ 均低于 deadline 50%。192 kHz × 8 声道（65k taps × 8 实例）stable≈51%、transition≈99%，是共享 8 实例容量的极端边界，不据此再对用户展示固定时长/实例性能橙色警告。prepare 全程亚 2 ms 量级，bypassed 路径可忽略。产品不再按 IR 秒数或 EQ 实例数弹出“性能可能下降”提示；用户以实际听感与系统负载为准。
