...
结论：在 M1 级机器上，1/2/4/8 声道的 48/96 kHz 与 1/2/4 声道的 192 kHz dense EQ 均低于 deadline 50%。192 kHz × 8 声道（65k taps × 8 实例）stable≈51%、transition≈99%，是共享 8 实例容量的极端边界，不据此对常见立体声路径告警。Convolution IR 性能提示门限由 8 秒上调至 **30 秒**（见 ADR-0017）：dense ~9 s 已测量可跑，8 秒门限过紧。prepare 全程亚 2 ms 量级，bypassed 路径可忽略。
